...
DECLARE
  ...
  v_jobId VARCHAR2(30) := NULL;
	...
	v_projectName VARCHAR2(30) := 'Json_Projects';
...
BEGIN
	...
	IF v_obj_cnt > 0 THEN
      ...
      v_workflow_name := '01_WorldBank_Json';
     	SELECT COUNT(1) INTO v_obj_cnt FROM ODMR_USER_PROJECT_WORKFLOW
  ...
  v_workflow_name := '01_WorldBank_Json';
	v_wfId := ODMRSYS.ODMR_WORKFLOW.WF_IMPORT(
                        p_project_id => v_projectId,
											  ...
											  p_workflow_data => XMLType(bfilename('ZIRAAT_DM_DIR', 'Json_01_WorldBank_Json.xml'), nls_charset_id('AL32UTF8')),
											  p_comment => 'WorldBank Data Mart Preperation');
