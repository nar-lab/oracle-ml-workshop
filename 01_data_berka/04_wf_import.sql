...
DECLARE
  ...
  v_jobId VARCHAR2(30) := NULL;
	...
	v_projectName VARCHAR2(30) := 'Berka_Banking';
...
BEGIN
	...
	IF v_obj_cnt > 0 THEN
      ...
      v_workflow_name := '01_DataPrep_Account';
     	SELECT COUNT(1) INTO v_obj_cnt FROM ODMR_USER_PROJECT_WORKFLOW
      ...
      v_workflow_name := '02_DataPrep_Transaction';
     	SELECT COUNT(1) INTO v_obj_cnt FROM ODMR_USER_PROJECT_WORKFLOW
    			WHERE PROJECT_NAME = v_projectName AND WORKFLOW_NAME = v_workflow_name;
  ...
  v_projectId := ODMRSYS.ODMR_PROJECT.PROJECT_CREATE(p_project_name => v_projectName, p_comment => 'Berka DB Clustering Solution');
  ...
  v_workflow_name := '01_DataPrep_Account';
	v_wfId := ODMRSYS.ODMR_WORKFLOW.WF_IMPORT(
                        p_project_id => v_projectId,
											  ...
											  p_workflow_data => XMLType(bfilename('ZIRAAT_DM_DIR', 'Berka_01_DataPrep_Account.xml'), nls_charset_id('AL32UTF8')),
											  p_comment => 'Account Based Data Mart Preperation');
  ...
  v_wfId := ODMRSYS.ODMR_WORKFLOW.WF_IMPORT(
                        p_project_id => v_projectId,
											  p_workflow_name => v_workflow_name,
											  p_workflow_data => XMLType(bfilename('ZIRAAT_DM_DIR', 'Berka_02_DataPrep_Transaction.xml'), nls_charset_id('AL32UTF8')),
											  p_comment => 'Account Based Transaction Data Mart Preperation');
	DBMS_OUTPUT.PUT_LINE('Workflow: '||v_wfId);
  
  v_node := 'BERKA_ACCOUNT_DM';
  v_workflow_name := '01_DataPrep_Account';
  v_nodes.extend();
